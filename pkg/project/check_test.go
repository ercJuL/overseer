...
func TestGoModCheck(t *testing.T) {
	if err := ioutil.WriteFile("./test.mod", []byte(testModContent), os.ModePerm); err != nil {
		t.Error(err)
		return
	}
	checkResult, err := GoModCheck("./test.mod", "k8s.io", "gopkg.in")
	if err != nil {
		t.Error(err)
		return
	}
	if len(checkResult.AutoUpdateVersionMods) != 4 {
		t.Errorf("自动更新的私有模块数量不符 应 4， 实际 %d", len(checkResult.AutoUpdateVersionMods))
	}
	_ = os.Remove("./test.mod")
}
